...
func TestIcatifyCloseRightAway(t *testing.T) {
	w, err := NewWatcher()
	if err != nil {
...
func TestIcatifyCloseSlightlyLater(t *testing.T) {
	w, err := NewWatcher()
	if err != nil {
		t.Fatalf("Failed to create watcher")
	}
...
func TestIcatifyCloseSlightlyLaterWithWatch(t *testing.T) {
	testDir := tempMkdir(t)
...
func TestIcatifyCloseAfterRead(t *testing.T) {
	testDir := tempMkdir(t)
	defer os.RemoveAll(testDir)
...
func TestIcatifyCloseCreate(t *testing.T) {
	testDir := tempMkdir(t)
	defer os.RemoveAll(testDir)
...
// when under load.
func TestIcatifyStress(t *testing.T) {
	maxNumToCreate := 1000
	...
	go func() {
		...
		// If we delete a newly created file too quickly, icatify will skip the
...
func TestIcatifyRemoveTwice(t *testing.T) {
	testDir := tempMkdir(t)
	defer os.RemoveAll(testDir)
...
func TestIcatifyInnerMapLength(t *testing.T) {
	testDir := tempMkdir(t)
	defer os.RemoveAll(testDir)
	testFile := filepath.Join(testDir, "testfile")
...
func TestIcatifyOverflow(t *testing.T) {
	// We need to generate many more events than the
	// fs.icatify.max_queued_events sysctl setting.
