...
func tempMkdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "fscatify")
	if err != nil {
...
func tempMkFile(t *testing.T, dir string) string {
	f, err := ioutil.TempFile(dir, "fscatify")
	if err != nil {
...
// newWatcher initializes an fscatify Watcher instance.
...
func TestFscatifyMultipleOperations(t *testing.T) {
	...
	testFile := filepath.Join(testDir, "TestFscatifySeq.testfile")
	testFileRenamed := filepath.Join(testDirToMoveFiles, "TestFscatifySeqRename.testfile")
	...
	// Create a file
	// This should add at least one event to the fscatify event queue
	var f *os.File
	...
	// Try closing the fscatify instance
	t.Log("calling Close()")
	watcher.Close()
...
func TestFscatifyMultipleCreates(t *testing.T) {
	watcher := newWatcher(t)
...
func TestFscatifyDirOnly(t *testing.T) {
	watcher := newWatcher(t)
	...
	// This should NOT add any events to the fscatify event queue
	testFileAlreadyExists := filepath.Join(testDir, "TestFscatifyEventsExisting.testfile")
	{
		var f *os.File
	...
	testFile := filepath.Join(testDir, "TestFscatifyDirOnly.testfile")
...
func TestFscatifyDeleteWatchedDir(t *testing.T) {
	watcher := newWatcher(t)
	defer watcher.Close()
...
func TestFscatifySubDir(t *testing.T) {
	watcher := newWatcher(t)
	...
	testFile1 := filepath.Join(testDir, "TestFscatifyFile1.testfile")
	testSubDir := filepath.Join(testDir, "sub")
	testSubDirFile := filepath.Join(testDir, "sub/TestFscatifyFile1.testfile")
...
func TestFscatifyRename(t *testing.T) {
	watcher := newWatcher(t)
	...
	testFile := filepath.Join(testDir, "TestFscatifyEvents.testfile")
	testFileRenamed := filepath.Join(testDir, "TestFscatifyEvents.testfileRenamed")
	...
	if renameReceived.value() == 0 {
		t.Fatal("fscatify rename events have not been received after 500 ms")
	}
...
func TestFscatifyRenameToCreate(t *testing.T) {
	watcher := newWatcher(t)
	...
	testFile := filepath.Join(testDirFrom, "TestFscatifyEvents.testfile")
	testFileRenamed := filepath.Join(testDir, "TestFscatifyEvents.testfileRenamed")
	...
	if createReceived.value() == 0 {
		t.Fatal("fscatify create events have not been received after 500 ms")
	}
...
func TestFscatifyRenameToOverwrite(t *testing.T) {
	switch runtime.GOOS {
	case "plan9", "windows":
	...
	if eventReceived.value() == 0 {
		t.Fatal("fscatify events have not been received after 500 ms")
	}
...
func TestFscatifyAttrib(t *testing.T) {
	...
	testFile := filepath.Join(testDir, "TestFscatifyAttrib.testfile")
	...
	if attribReceived.value() == 0 {
		t.Fatal("fscatify attribute events have not received after 500 ms")
	}
...
func TestFscatifyClose(t *testing.T) {
	watcher := newWatcher(t)
	watcher.Close()
...
func TestFscatifyFakeSymlink(t *testing.T) {
	if runtime.GOOS == "windows" {
	...
	if errorsReceived.value() > 0 {
		t.Fatal("fscatify errors have been received.")
	}
	if otherEventsReceived.value() > 0 {
		t.Fatal("fscatify other events received on the broken link")
	}

	// Except for 1 create event (for the link itself)
	if createEventsReceived.value() == 0 {
		t.Fatal("fscatify create events were not received after 500 ms")
	}
	if createEventsReceived.value() > 1 {
		t.Fatal("fscatify more create events received than expected")
	}
