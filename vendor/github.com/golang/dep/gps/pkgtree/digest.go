...
const (
	// CATInLock is used when a file system node exists for which there is no
	// corresponding dependency in the lock file.
	CATInLock VendorStatus = iota

	// CATInTree is used when a lock file dependency exists for which there is
...
func VerifyDepTree(osDirname string, wantSums map[string][]byte) (map[string]VendorStatus, error) {
	...
	// prefer the output to state that `github.com/bob` is `CATInLock`, and
	// `launchpad.net/nifty` is `CATInLock`.
	//
	// 2) If `alice1`, `alice2`, and `bob1` were in the lock file, we'd want to
	// report `github.com/bob/bob2` as `CATInLock`, and `launchpad.net/nifty` is
	// `CATInLock`.
	//
	// 3) If none of `alice1`, `alice2`, `bob1`, or `bob2` were in the lock
	// file, the entire `github.com` directory would be reported as `CATInLock`,
	// along with `launchpad.net/nifty` is `CATInLock`.
	//
	...
	for slashPathname := range wantSums {
		slashStatus[slashPathname] = CATInTree
	}
	...
	// current node ought to be marked as `CATInLock`.
	for len(nodes) > 1 {
		...
		if !currentNode.isRequiredAncestor && nodes[currentNode.parentIndex].isRequiredAncestor {
			slashStatus[filepath.ToSlash(currentNode.osRelative)] = CATInLock
		}
	}
