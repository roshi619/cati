...
type Viper struct {
	...
	onConfigChange func(fscatify.Event)
}
...
func OnConfigChange(run func(in fscatify.Event)) { v.OnConfigChange(run) }
func (v *Viper) OnConfigChange(run func(in fscatify.Event)) {
			...
			for {
				...
				case event := <-watcher.Events:
					...
					if filepath.Clean(event.Name) == configFile {
						if event.Op&fscatify.Write == fscatify.Write || event.Op&fscatify.Create == fscatify.Create {
							err := v.ReadInConfig()
							if err != nil {
								log.Println("error:", err)
							}
